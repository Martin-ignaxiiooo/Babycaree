 ...
 */
export const auditAction = (tablaAfectada: string) => {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        ...
        if (req.admin) {
          logAudit(
            ...
            tablaAfectada,
            idRegistro,
            null, // valor anterior (sería ideal pasarlo pero req no lo tiene por defecto)
            body, // valor nuevo
            req.ip || "unknown"
          ).catch(err => console.error("Fallo de auditoría asíncrona:", err));
        }
      }
