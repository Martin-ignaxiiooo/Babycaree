...
exports.sincronizarContactos = exports.buscarPersonas = void 0;
    ...
    try {
        ...
        if (fuente === 'perfiles') {
            ...
            if (queryText.length > 0) {
                const result = await (0, db_1.query)(`SELECT DISTINCT u.id, u.nombre, u.apellidos, u.email 
           ...
           LIMIT 5`, [userId, `%${queryText}%`, queryText]);
                results = result.rows;
            }
        }
            ...
            if (isEmail) {
                const result = await (0, db_1.query)(`SELECT id, nombre, apellidos, email FROM usuarios WHERE email = $1 AND id != $2`, [queryText, userId]);
                results = result.rows;
            }
            else {
...
const sincronizarContactos = async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const result = await (0, db_1.query)(`SELECT id, nombre, apellidos, email 
       FROM usuarios 
       WHERE (correo_hash = ANY($1) OR telefono_hash = ANY($1)) AND id != $2`, [hashes, userId]);
...
exports.sincronizarContactos = sincronizarContactos;
