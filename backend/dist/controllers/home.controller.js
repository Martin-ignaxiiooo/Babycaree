...
exports.addGrowthRecord = exports.getHomeDashboard = void 0;
...
function calculateApproxPercentile(weight, dob) {
    if (!weight || weight === 0)
        return 50;
    const birth = new Date(dob);
    const now = new Date();
    ...
    if (zScore < -1)
        return 15;
    if (zScore < 0)
        return Math.max(15, Math.round(50 + (zScore * 35)));
    if (zScore < 1)
        return Math.min(85, Math.round(50 + (zScore * 35)));
    if (zScore < 2)
        return 85;
    if (zScore < 2.5)
        return 97;
...
const getHomeDashboard = async (req, res) => {
    try {
        ...
        const profileRes = await (0, db_1.query)(`SELECT b.*, t.nombre_visible as nombre_prevision 
       FROM perfiles_bebes b 
       LEFT JOIN tipos_prevision t ON b.prevision_salud = t.codigo 
       WHERE b.id = $1`, [idPerfil]);
       ...
       WHERE bebe_id = $1 
       ORDER BY fecha_registro DESC LIMIT 1`, [idPerfil]);
        ...
        const vacunasRes = await (0, db_1.query)(`SELECT v.nombre, rv.fecha_aplicacion
       FROM registro_vacunas rv
       JOIN vacunas_pni v ON rv.vacuna_id = v.id
       WHERE rv.bebe_id = $1 AND rv.aplicada = FALSE
       ORDER BY rv.fecha_aplicacion ASC`, [idPerfil]);
        vacunasRes.rows.forEach((v) => {
        ...
        // B. Citas Médicas
        const citasRes = await (0, db_1.query)(`SELECT especialidad, fecha_cita, medico
       FROM citas_medicas
       WHERE bebe_id = $1 AND estado = 'programada' AND fecha_cita > NOW()
       ORDER BY fecha_cita ASC`, [idPerfil]);
        citasRes.rows.forEach((c) => {
        ...
        const historyRes = await (0, db_1.query)(`SELECT fecha_registro, peso_kg, talla_cm 
       FROM registros_crecimiento 
       WHERE bebe_id = $1 
       ORDER BY fecha_registro ASC`, [idPerfil]);
        ...
        for (const mes of sortedMonths) {
            ...
            const omsRes = await (0, db_1.query)(`SELECT peso_esperado_kg FROM oms_percentiles WHERE mes_vida = $1 AND (sexo = $2 OR sexo = 'Unisex') ORDER BY sexo DESC LIMIT 1`, [mes, omsSexo]);
...
exports.addGrowthRecord = addGrowthRecord;
