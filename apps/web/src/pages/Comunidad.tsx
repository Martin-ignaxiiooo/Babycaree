      ...
      <div className="page-container" style={{ marginTop: "-14px" }}>
        ...
        <div className="responsive-overflow comunidad-tabs-row" style={{ display: "flex", gap: "10px", marginBottom: "32px", borderBottom: "1px solid var(--theme-bg-light)", paddingBottom: "16px", whiteSpace: "nowrap" }}>
          <button 
            ...
            style={{ 
              ...
              color: activeTab === "foros" ? "#fff" : "var(--text-muted)",
          ...
          <button 
            ...
            style={{ 
              ...
              color: activeTab === "articulos" ? "#fff" : "var(--text-muted)",
              border: "none", padding: "12px 22px", borderRadius: "100px", 
              fontWeight: 800, cursor: "pointer", fontSize: "14.5px",
              display: "flex", alignItems: "center", gap: "8px", transition: "all 0.2s",
              ...
              <div key={foro.id} onClick={() => navigate(`/comunidad/foro/${foro.id}`)} style={{ 
                ...
                <div style={{ flex: 1, minWidth: "300px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "8px" }}>
                    ...
                    <span style={{ fontSize: "13px", color: "var(--text-muted)" }}>Por {foro.autor_nombre} · {foro.tiempo_publicacion}</span>
                  </div>
                ...
                <div style={{ display: "flex", gap: "24px", color: "var(--text-muted)" }}>
                  <div 
                    ...
                    style={{ 
                      ...
                      color: foro.has_liked ? "var(--theme-primary)" : "var(--text-muted)",
                  ...
                  <ChevronRight size={24} style={{ color: "var(--border-strong, var(--text-muted))" }} />
                </div>
              </div>
            ))}
          </div>
        ...
        {activeTab === "articulos" && (
          <div className="responsive-grid articulos-grid">
            {loading ? <div style={{ padding: "40px", color: "var(--text-muted)", gridColumn: "1/-1", textAlign: "center" }}>Cargando artículos...</div> : articulosData.map(art => (
              <div key={art.id} className="articulo-card" onClick={() => navigate(`/comunidad/articulo/${art.id}`)} style={{ 
                ...
                <div className="articulo-body" style={{ padding: "24px", flex: 1, display: "flex", flexDirection: "column" }}>
                  ...
                  <p className="articulo-resumen" style={{ margin: "0 0 8px 0", fontSize: "14px", color: "var(--text-muted)", lineHeight: "1.5", flex: 1 }}>
                  ...
                  <div className="articulo-footer" style={{ display: "flex", alignItems: "center", justifyContent: "flex-start", gap: "10px" }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        marginTop: "8px",
                        color: art.has_liked ? "var(--theme-primary)" : "var(--text-muted)",
                      }}
                    >
          ...
          <div style={{ background: "var(--surface)", borderRadius: "24px", width: "100%", maxWidth: "600px", padding: "32px", boxShadow: "0 20px 40px rgba(0,0,0,0.2)" }}>
            ...
            <form onSubmit={handleCrearPost}>
              <div style={{ marginBottom: "16px" }}>
                <label style={{ display: "block", fontSize: "14px", fontWeight: 700, color: "var(--text)", marginBottom: "8px" }}>Título</label>
                <input 
                  type="text" required value={nuevoTitulo} onChange={e => setNuevoTitulo(e.target.value)}
                  placeholder="¿Cuál es tu duda o experiencia?"
                  style={{ width: "100%", padding: "12px 16px", border: "1px solid var(--border)", borderRadius: "12px", fontSize: "15px", outline: "none" }}
                />
              </div>

              <div style={{ marginBottom: "16px" }}>
                <label style={{ display: "block", fontSize: "14px", fontWeight: 700, color: "var(--text)", marginBottom: "8px" }}>Categoría</label>
                <select 
                  value={nuevaCategoria} onChange={e => setNuevaCategoria(e.target.value)}
                  style={{ width: "100%", padding: "12px 16px", border: "1px solid var(--border)", borderRadius: "12px", fontSize: "15px", outline: "none", background: "var(--surface)" }}
                >
              ...
              <div style={{ marginBottom: "24px" }}>
                <label style={{ display: "block", fontSize: "14px", fontWeight: 700, color: "var(--text)", marginBottom: "8px" }}>Contenido</label>
                <textarea 
                  required value={nuevoContenido} onChange={e => setNuevoContenido(e.target.value)}
                  placeholder="Escribe los detalles aquí..." rows={5}
                  style={{ width: "100%", padding: "12px 16px", border: "1px solid var(--border)", borderRadius: "12px", fontSize: "15px", outline: "none", resize: "none" }}
                />
              </div>

              <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end" }}>
                <button type="button" onClick={() => setShowModal(false)} style={{ background: "none", border: "none", color: "var(--text-muted)", fontWeight: 700, padding: "12px 24px", cursor: "pointer", borderRadius: "12px" }}>
                  Cancelar
                </button>
